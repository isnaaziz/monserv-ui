import { Search, Bell, Sun } from 'lucide-react';
import { Input } from '../ui/Input';
import { Button } from '../ui/Button';
import { Avatar } from '../ui/Avatar';

export function Topbar() {
    return (
        <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-zinc-950/50 border-b border-zinc-900/50 px-6 py-4 flex items-center justify-between">
            {/* Search */}
            <div className="hidden md:block w-64 lg:w-96">
                <Input
                    placeholder="Search resources..."
                    icon={Search}
                    className="bg-zinc-900/50 border-zinc-800"
                />
            </div>

            {/* Actions */}
            <div className="flex items-center gap-3 ml-auto">
                <Button variant="ghost" size="icon" className="relative text-zinc-400 hover:text-zinc-100">
                    <Bell className="size-5" />
                    <span className="absolute top-2 right-2 size-2 bg-red-500 rounded-full border-2 border-zinc-950" />
                </Button>

                <Button variant="ghost" size="icon" className="text-zinc-400 hover:text-zinc-100">
                    <Sun className="size-5" />
                </Button>

                <div className="h-6 w-px bg-zinc-800 mx-2" />

                <button className="flex items-center gap-3 pl-2 group">
                    <Avatar src="https://ui-avatars.com/api/?name=Arun+Kumar&background=10b981&color=fff" />
                    <div className="hidden lg:flex flex-col items-start">
                        <span className="text-sm font-medium text-zinc-200 group-hover:text-white transition-colors">Arun Kumar</span>
                        <span className="text-xs text-zinc-500">Super Admin</span>
                    </div>
                </button>
            </div>
        </header>
    );
}
